...
class UsuarioServicio:
    ...
    """
    def __init__(self, usuario_repositorio: IUsuarioRepositorio, hasher: Hasher):
        # El servicio depende de la INTERFAZ del repositorio, no de la implementación concreta
    ...
    async def crear_usuario(self, email: str, password: str, full_name: str) -> Usuario:
        ...
        usuario_existente = await self.usuario_repositorio.get_by_email(email_obj.valor)
        ...
        usuario_creado = await self.usuario_repositorio.save(nuevo_usuario)
    ...
    async def obtener_todos_los_usuarios(self, skip: int = 0, limit: int = 100) -> List[Usuario]:
        ...
        return await self.usuario_repositorio.get_all(skip=skip, limit=limit)
    ...
    async def actualizar_usuario(
        ...
        """
        usuario = await self.obtener_usuario_por_id(user_id)
        updated_at_original = usuario.updated_at

        if email is not None:
            email_obj = CorreoElectronico(email)
            if email_obj.valor != usuario.email:
                otro_usuario = await self.usuario_repositorio.get_by_email(email_obj.valor)
        ...
        if usuario.updated_at != updated_at_original:
            return await self.usuario_repositorio.save(usuario)
    ...
    async def eliminar_usuario(self, user_id: UUID) -> None:
        ...
        await self.obtener_usuario_por_id(user_id)
        await self.usuario_repositorio.delete(user_id)
    ...
    async def verificar_credenciales(self, email: str, password: str) -> Usuario:
        ...
        """
        email_obj = CorreoElectronico(email)
        usuario = await self.usuario_repositorio.get_by_email(email_obj.valor)
    ...
    async def cambiar_contrasena(self, user_id: UUID, contrasena_actual: str, nueva_contrasena: str) -> None:
        ...
        await self.usuario_repositorio.save(usuario)